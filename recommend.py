...
#Pearson correlation similarity matrix#Pearso
...
# This function predicts the rating for specified user-item combination based on item-based approach#This fu
...
def findksimilaritems_adjcos(item_id, ratings, k=k):
    ...
    print
    ('{0} most similar items for item {1}:\n'.format(k, item_id))
    for i in range(0, len(indices)):
        if indices[i] + 1 == item_id:
            ...
            print
            ('{0}: Item {1} :, with similarity of {2}'.format(i, indices[i] + 1, similarities[i]))

    return similarities, indices

similaritiessimilari , indices = findksimilaritems_adjcos(3,M)
...
#check for incorrect entries#check f 
recommendItem(-1,3,M)
...
def evaluateRS(ratings):
    ...
    def on_change(change):
        ...
        print
        ("RMSE using {0} approach is: {1}".format(approach.value, RMSE))
